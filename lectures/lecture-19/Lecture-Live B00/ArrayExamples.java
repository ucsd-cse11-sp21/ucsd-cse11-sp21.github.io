...
class ArrayExamples {
  ...
  void testCountTweetsWithHashtag(Tester t) {
  ...
  double length1 = averageLengthOfTweetsWithHashtag(tweets, "ucsd");
  double length1Expected = (t2.contents.length() + t4.contents.length()) / 2.0;
  double length2 = averageLengthOfTweetsWithHashtag(tweets, "welcome");
  double length2Expected = 
    (t1.contents.length() + t2.contents.length() + t3.contents.length()) / 3.0;
  double length3 = averageLengthOfTweetsWithHashtag(tweets, "batman");

  //How would we access the contents of the third tweet in the tweets array
  //(without writing t3 in our code)
  String contentsOfT3 = tweets[2].contents;

  //What would we write to get the sttring "jacobsschool" below using just array
  //lookup and field lookup:
  String userOfT2 = tweets[1].user;

  //Whwat would we write to get the length of the contents of the t4 tweet:
